...
def contrast_analysis(datatable, contrast):
    ...
    for col in datatable.columns[3:]:
        SNC_mean = datatable.groupby(['groups']).mean().loc[:,col].to_numpy()[0]
        NC_mean = datatable.groupby(['groups']).mean().loc[:,col].to_numpy()[1]
        MCI_mean = datatable.groupby(['groups']).mean().loc[:,col].to_numpy()[2]
        AD_mean = datatable.groupby(['groups']).mean().loc[:,col].to_numpy()[3]
        ...
        print(f"The {col} contrast has F_value {F_value}, and the F_critical is {F_critical}")
