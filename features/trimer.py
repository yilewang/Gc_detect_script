...
if __name__ == "__main__":
    Trimer_Homo = pd.DataFrame(columns=['groups','caseid', 'trimer_homo','aCNG','mCNG','pCNG','HIP','PHG','AMY','sTEMp','mTEMp' ])
    Trimer_Hetero  = pd.DataFrame(columns=['groups','caseid', 'trimer_hetero','aCNG','mCNG','pCNG','HIP','PHG','AMY','sTEMp','mTEMp' ])

    for grp in groups:
        # subject case ids
        ldir = os.listdir("C:/Users/Wayne/tvb/TS-4-Vik/"+ grp+'-TS')
        # ldir = os.listdir('/home/wayne/TS-4-Vik/'+grp+'-TS/')
        MC_heat = np.zeros((16,16, len(ldir)))
        for ind, y in enumerate(ldir):
            ...
            for idnx, i in enumerate(range(0,15,2)):
                ...
                for idx, restNode in enumerate(newList):
                    MC14[0,idx] = MC_Trimers[i,j,restNode] # In rest of the 14 regions, iternate the third dimensions, and pick up the homotopic MC
                tmp_homo = np.average(MC14) # averging the 14 results
                homoRegions[0,idnx] = tmp_homo
            Trimer_Homo = Trimer_Homo.append({'groups':grp,'caseid':y,'trimer_homo':np.mean(homoRegions), 'aCNG':homoRegions[0,0],'mCNG':homoRegions[0,1],'pCNG':homoRegions[0,2],'HIP':homoRegions[0,3],'PHG':homoRegions[0,4], 'AMY':homoRegions[0,5],'sTEMp':homoRegions[0,6],'mTEMp':homoRegions[0,7]},ignore_index=True)
    Trimer_Homo.to_excel("meta_homo.xlsx")
